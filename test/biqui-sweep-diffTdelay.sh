...
#===============================================================================
#
#          FILE: biqui-sweep-diffTdelay.sh
# 
#         USAGE: ./biqui-sweep-diffTdelay.sh 
# 
#   DESCRIPTION: 
# 
#       OPTIONS: ---
...
results_dir="results/journal-extension/diffTdelays"
...
results_cf="$results_dir/cloudFirst-biqui-targetd_$target_d-reliab_$reliab-mode_$mode-$now.csv"
results="$results_dir/biqui-targetd_$target_d-reliab_$reliab-mode_$mode-$now.csv"
...
for lam in `seq $lam_min $lam_step $lam_max`; do
    ...
    echo python3 biqui.py --mu $mu --lambda_ $lam --target_d $target_d --percentile $reliab --c0c $c0c --c1c $c1c --cost_ratio $ecratio --edge_d $del_edge --cost_fn $cost_fn --cloud_d $del_cloud --subs_ratio $subsratio
    ...
    # Cloud first BiQui
    log_cf="/tmp/logs/cloudFirst-biqui-reliab-$reliab-targetdel-$target_d-lambda-`printf '%.2f' $lam`-mu-`printf '%.2f' $mu`-mode_$mode.log"
    echo Working on $log_cf
    python3 biqui.py\
        --mu $mu --lambda_ $lam\
        --target_d $target_d --percentile $reliab\
        --c0c $c0c --c1c $c1c --cost_ratio $ecratio\
        --edge_d $del_edge --cost_fn $cost_fn\
        --cloud_d $del_cloud\
        --subs_ratio $subsratio -cf &> $log_cf
    tail -n1 $log_cf >> $results_cf
