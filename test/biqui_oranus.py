...
def bin_search(c, lambda_, mu, percentile, tgt, delay, avg_d=False):
    ...
    if float(f'{rho:.2f}') <= 0.95:
        f = f'../results/cdf-oranus/rho-{rho:.2f}_c-{c}.csv'
    ...
    while c != c_next:
        ...
        if rho_next <= 0.95:
            ...
            f = f'../results/cdf-oranus/rho-{rho_next:.2f}_c-{c_next}.csv'
...
if __name__ == '__main__':
    ...
    while z>0:
        ...
        f = f'../results/cdf-oranus/rho-{rho_e:.2f}_c-{ce}.csv'
        print('  Loading', f)
        df_next = pd.read_csv(f, names=['x', 'cdf'], header=None)
        df_next = df_next.dropna()
        ...
        while perc_c >= args.percentile and cc > 0:
            ...
            f = f'../results/cdf-oranus/rho-{rho_c:.2f}_c-{cc}.csv'
            print('  Loading', f)
            df_next = pd.read_csv(f, names=['x', 'cdf'], header=None)
            df_next = df_next.dropna()
