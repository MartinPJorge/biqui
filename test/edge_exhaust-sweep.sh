...
#===============================================================================
#
#          FILE: edge_exhaust-sweep.sh
# 
#         USAGE: ./edge_exhaust-sweep.sh 
# 
#   DESCRIPTION: 
# 
#       OPTIONS: ---
...
results="edge_exhaust-targetd_$target_d-reliab_$reliab-mode_$mode-$now.csv"
...
for lam in `seq $lam_min $lam_step $lam_max`; do
    log_f="logs/edge_exhaust-reliab-$reliab-targetdel-$target_d-lambda-`printf '%.2f' $lam`-mu-`printf '%.2f' $mu`-mode_$mode.log"
    echo Working on $log_f
    python3 edge_exhaust.py\
        --mu $mu --lambda_ $lam\
