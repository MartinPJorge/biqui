...
if __name__ == '__main__':
    ...
    if not found:
        # Find max rho_e admitted w/ edge_max
        perc_e, rho_e = 0, 0.96
        rho_step = 0.01
        while perc_e < args.percentile:
            rho_e -= rho_step
        ...
        # Send the remaining load to the Cloud
        lambda_e = rho_e * args.edge_max * args.mu

        # lambda_e = (1-z) * lambda
        # take max, due to sweep granularity it may report z<0
        z = max(1 - lambda_e / args.lambda_, 0.01)

        lambda_c = z * args.lambda_

        # Find feasible setup for cloud offload
        found, cc = False, 1
        while not found and cc < args.cloud_max:
            rho_c = lambda_c / (cc*args.mu)
        ...
        if not found:
            ...
            if args.cost_fn == 'money':
                min_cost = cost(x=args.edge_max, y=cc,
            ...
            print(args.edge_max, cc, z, min_cost, args.lambda_, args.mu,
                    args.edge_d, args.cloud_d, args.target_d,
                    args.percentile, args.cost_ratio)
        ...
        if args.cost_fn == 'money':
            min_cost = cost(x=ce, y=0, z=0, lam=args.lambda_,
                            mu=args.mu, 
                            c0e=args.cost_ratio*args.c0c,
                            c0c=args.c0c,
                            c1e=args.subs_ratio*args.c1c,
        ...
        # & compute avg_del_e
            ...
            cdfs = {('a','a'): df_next}
            avg_del_e = avg_delay(cdfs, 'a', 'a')
            min_cost = avg_delay_cost(z=0, avg_del_e=avg_del_e,
                        avg_del_c=0)
        ...
        print(ce, 0, 0, min_cost, args.lambda_, args.mu,
                args.edge_d, args.cloud_d, args.target_d,
                args.percentile, args.cost_ratio)
