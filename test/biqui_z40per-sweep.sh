...
#===============================================================================
#
#          FILE: biqui_z40per-sweep.sh
# 
#         USAGE: ./biqui_z40per-sweep.sh 
# 
#   DESCRIPTION: 
# 
#       OPTIONS: ---
...
results="../results/sigmetrics2024/biqui_z40per-targetd_$target_d-reliab_$reliab-mode_$mode-$now.csv"
...
for lam in `seq $lam_min $lam_step $lam_max`; do
    log_f="/tmp/logs/biqui_z40per-reliab-$reliab-targetdel-$target_d-lambda-`printf '%.2f' $lam`-mu-`printf '%.2f' $mu`-mode_$mode.log"
    echo Working on $log_f
    python3 biqui_z40per.py\
        --mu $mu --lambda_ $lam\
